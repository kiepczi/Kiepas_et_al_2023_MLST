...
#Saving passed arguments to variables
colnumbering_file = sys.argv[1]
current_part_file = sys.argv[2]
save_new_part_file_to = sys.argv[3]
...
def update_part_file(colmatchfile, partfile):
    ...
    alg_col_match = open(Path(colmatchfile).expanduser(), 'r')
...
partition_file_no_gaps = update_part_file(f'{colnumbering_file}', f'{current_part_file}')

#we can save the partition
with open(Path(f"{save_new_part_file_to}").expanduser(), "w") as text_file:
