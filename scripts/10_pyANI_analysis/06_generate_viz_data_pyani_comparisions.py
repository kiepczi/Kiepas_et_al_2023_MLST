...
#Path to all files
datadir = Path("~/Desktop/Kiepas_et_al_2023_MLST/data/pyani_analysis/connected_components_matrices/").expanduser()
...
for i in range(1,118):
    #Loading data
    identity = pd.read_csv(Path(f"~/Desktop/Kiepas_et_al_2023_MLST/data/pyani_analysis/connected_components_matrices/matrix_identity_{i}.tab").expanduser(), sep='\t').rename(columns={'Unnamed: 0': 'genome1'})
    coverage = pd.read_csv(Path(f"~/Desktop/Kiepas_et_al_2023_MLST/data/pyani_analysis/connected_components_matrices/matrix_coverage_{i}.tab").expanduser(), sep='\t').rename(columns={'Unnamed: 0': 'genome1'})

    #Getting number of unique specie name per connected component
    unique_tax_names[i] = len(set([' '.join(_.split(' ')[0:2]) for _ in identity if _ != 'genome1']))
    #Melting data
...
data.to_csv(Path("~/Desktop/Kiepas_et_al_2023_MLST/data/pyani_analysis/pyani_coevarge_identity_df.csv").expanduser(), index=False)
...
MLST_data_info = pd.read_csv(Path("~/Desktop/Kiepas_et_al_2023_MLST/data/MLST_scheme_revision/Genomes_ST_info.csv").expanduser())
...
ST_data.to_csv(Path("~/Desktop/Kiepas_et_al_2023_MLST/data/pyani_analysis/pyani_coevarge_identity_ST_comparisions_df.csv").expanduser(), index=False)
