...
save_genome_info_to = sys.argv[4]

#Load data. Mlst (v2.19.0) output
df = pd.read_csv(f'{most_recent_mlst_output}', sep='\t', usecols=['FILE', 'SCHEME', 'ST', '16S', 'atpD', 'gyrB', 'recA', 'rpoB', 'trpB'])
...
#Drop columns that have more than 2 diffrent copies of an allele
...
pubmlst_new_profiles = pubmlst_new_profiles[(pubmlst_new_profiles['ST'] == 0)]



# #Load existing profiles
latest_scheme = pd.read_csv(most_recent_mlst_scheme,sep='\t', dtype="object")
...
info_data.to_csv(f"{save_genome_info_to}", index=False)
